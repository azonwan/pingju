...
(function($){
    ...
    var Pingju = {
        ...
        */
        ...
        nodeClick: function(){
            $('.node_name').bind({
                click: function(){
                    ...
                    $.ajax({
                        url: post_url, 
                        data: {node_id:node_id,_token:_token},
                        dataType: 'html',
                        type: 'POST',
                        success: function(data){
                          $('.topic-content').html(data);  
                          Pingju.initDeleteForm();
                        }
                    });
                }
            });
        ...
        topicClick: function(){
            $('.topic-content').delegate('li[name=topic_li]','click',function(){
                var topic_id = $(this).attr('topic_id');
                var _token = Config.token;
                var data_url = $(this).attr('data-url');
                $('li[name=topic_li]').removeClass('li-active');
        ...
        operateClick: function(){
            $('.updated-card .card').delegate('a','click',function(){
                var _this = $(this);
                var node_id = $(this).attr('node_id');
                var _token = Config.token;
                var old_class = $(this).attr('class');
                var operate_type = old_class;
                var data_url = $(this).attr('data_url');
                var replace_class = (operate_type == 'follow') ? 'unfollow' : 'follow';
                $.ajax({
                    url: data_url, 
                    data: {node_id:node_id,_token:_token,operate_type:operate_type},
                    dataType: 'json',
                    type: 'POST',
                    success: function(data){
                        _this.removeClass(old_class).addClass(replace_class);
                        _this.text(Config.variable[replace_class]);
                    }
                });
            });
        }
    }
    window.Pingju = Pingju;
})(jQuery);
